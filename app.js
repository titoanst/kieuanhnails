...
form.onsubmit=async e=>{e.preventDefault();const submit=form.querySelector('[type="submit"]'),data=Object.fromEntries(new FormData(form));submit.disabled=true;submit.textContent='Đang gửi yêu cầu…';try{const response=await fetch('/api/booking',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)});if(!response.ok)throw new Error('send_failed');form.hidden=true;success.hidden=false;document.querySelectorAll('.modal-progress i').forEach(x=>x.classList.add('active'));form.reset()}catch{alert('Chưa gửi được yêu cầu. Anh/chị vui lòng thử lại hoặc liên hệ Zalo 0878 804 489.')}finally{submit.disabled=false;submit.textContent='Gửi yêu cầu đặt lịch →'}};
