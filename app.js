...
form.onsubmit=async e=>{e.preventDefault();const submit=form.querySelector('[type="submit"]'),data=Object.fromEntries(new FormData(form)),payload=JSON.stringify(data),send=()=>fetch('/api/booking',{method:'POST',headers:{'content-type':'application/json'},body:payload});submit.disabled=true;submit.textContent='Đang gửi yêu cầu…';try{let response;for(let attempt=0;attempt<2;attempt++){try{response=await send()}catch{}if(response?.ok)break;if(attempt===0)await new Promise(resolve=>setTimeout(resolve,900))}if(!response?.ok)throw new Error('send_failed');form.hidden=true;success.hidden=false;document.querySelectorAll('.modal-progress i').forEach(x=>x.classList.add('active'));form.reset();startedAt.value=Date.now()}catch{alert('Chưa gửi được yêu cầu. Anh/chị vui lòng thử lại hoặc liên hệ Zalo 0878 804 489.')}finally{submit.disabled=false;submit.textContent='Gửi yêu cầu đặt lịch →'}};
